...
/// <summary>
/// Handling of SupplyHandlingParameters command
/// </summary>
public class TSupplyHandlingParametersCommand : TAbstractLogicScriptObject
{
  ...
  {
    // Read Arguments
    int iParameterClass = CallContext.getInputArgumentValue<int>("parameterClass");
    string strTypeId = CallContext.getInputArgumentValue<string>("typeId");
    string strParameters = CallContext.getInputArgumentValue<string>("parameters");
    TGenericEntity[] aApplicationData = CallContext.getInputArgumentValue<TGenericEntity[]>("applicationData");

    Logger.writeInfo($"SupplyHandlingParametersCommand: '{DeviceNode.Name}'.");
    Logger.writeInfo($"  iParameterClass:  {iParameterClass}");
    Logger.writeInfo($"  strTypeId:        {strTypeId}");
    Logger.writeInfo($"  strParameters:    {strParameters}");
