...
/// <summary>
/// Handling of StartProductionProgram command
/// </summary>
public class TStartProductionProgramCommand : TAbstractLogicScriptObject
{
  ...
  {
    // Read Arguments
    string strProgramName = CallContext.getInputArgumentValue<string>("programPath");
    TGenericEntity[] aWorkpieceLocation = CallContext.getInputArgumentValue<TGenericEntity[]>("workpieceLocation");
    bool bAutomaticStart = CallContext.getInputArgumentValue<bool>("automaticStart");
    TGenericEntity[] aApplicationData = CallContext.getInputArgumentValue<TGenericEntity[]>("applicationData");

    Logger.writeInfo($"StartProductionProgramCommand: '{DeviceNode.Name}'.");
    Logger.writeInfo($"  strProgramPath:     {strProgramName}");
    Logger.writeInfo($"  aWorkpieceLocation: {printEntities(aWorkpieceLocation)}");
    Logger.writeInfo($"  bAutomaticStart:    {bAutomaticStart}");
