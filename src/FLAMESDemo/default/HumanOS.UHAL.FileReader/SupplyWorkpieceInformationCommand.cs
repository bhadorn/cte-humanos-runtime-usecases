...
/// <summary>
/// Handling of SupplyWorkpieceInformation command
/// </summary>
public class TSupplyWorkpieceInformationCommand : TAbstractLogicScriptObject
{
  ...
  public override async Task executeCommandAsync(IKernelAccess Kernel, IGroupRelation DeviceNode, ICommandCallContext CallContext, CancellationToken Token)
  {
    ...
    TGenericEntity[] aWorkpieceLocation = CallContext.getInputArgumentValue<TGenericEntity[]>("workpieceLocation");
    TGenericEntity Workpiece = CallContext.getInputArgumentValue<TGenericEntity>("workpiece");
    string strAdditionalParameters = CallContext.getInputArgumentValue<string>("additionalParameters");
    TGenericEntity[] aApplicationData = CallContext.getInputArgumentValue<TGenericEntity[]>("applicationData");

    Logger.writeInfo($"SupplyWorkpieceInformationCommand: '{DeviceNode.Name}'.");
    Logger.writeInfo($"  aWorkpieceLocation:      {printEntities(aWorkpieceLocation)}");
    Logger.writeInfo($"  Workpiece:               {printEntity(Workpiece)}");
    Logger.writeInfo($"  strAdditionalParameters: {strAdditionalParameters}");
