...
/// <summary>
/// Hanldling of the SupplyToolDataset command
/// </summary>
public class TSupplyToolDatasetCommand : TAbstractLogicScriptObject
{
  ...
  {
    string strDataset = CallContext.getInputArgumentValue<string>("dataset");
    TGenericEntity ToolId = CallContext.getInputArgumentValue<TGenericEntity>("toolId");
    TGenericEntity[] aApplicationData = CallContext.getInputArgumentValue<TGenericEntity[]>("applicationData");

    Logger.writeInfo($"SupplyToolDatasetCommand: '{DeviceNode.Name}'.");
    Logger.writeInfo($"  ToolId:           {printEntity(ToolId)}");
    Logger.writeInfo($"  strDataset:       {strDataset}");
