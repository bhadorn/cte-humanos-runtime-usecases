...
A **file-based simulator** is included (`C:\Temp\TestFile2.json`), so no physical hardware is required.

## Architecture

```text
JSON File  (C:\Temp\TestFile2.json)
...
- A JSON file at `C:\Temp\TestFile2.json` (create manually or copy the template from `DeviceTemplates/`)
