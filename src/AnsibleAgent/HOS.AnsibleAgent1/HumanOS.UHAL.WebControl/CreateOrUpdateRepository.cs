...
namespace HumanOS.IoT.Designer.Library.Scripts
{
  ...
  /// </summary>
  public class TCreateOrUpdateRepositoryScript : TAbstractHttpScriptObject
  {
                                          ...
                                          Dictionary<string, IEnumerable<string>> dicRequestHeaders)
    {
      ...
      int iRespositoryId = TCommandHelper.getArgumentOrDefault<int>(dicInputArguments, "RepositoryId", 0);

      JObject jRoot = new JObject();
      jRoot.Add(new JProperty("name", dicInputArguments["Name"]));
      jRoot.Add(new JProperty("git_url", dicInputArguments["Path"]));
      jRoot.Add(new JProperty("project_id", dicInputArguments["ProjectId"]));
      jRoot.Add(new JProperty("ssh_key_id", dicInputArguments["KeyId"]));
      
      if (iRespositoryId > 0)
      {
        jRoot.Add(new JProperty("id", iRespositoryId));
      }
